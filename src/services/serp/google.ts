...
export class GoogleSERP extends AsyncService {
    ...
    protected async createContext() {
        ...
        return {
            ...
            ua: this.getGsaUserAgent(),
        } as SerpContext;
    }

    gsaUserAgents: string[] = [];
    ...
    override async init() {
        ...
        this.gsaUserAgents = await readFile(resolve(__dirname, '../../../licensed/gsa_useragents.txt'), 'utf-8')
            .then((r) => r.split(/\r?\n/).map(l => l.trim()).filter(Boolean))
            .catch(err => {
                this.logger.error('Error reading GSA user agents file', { err });
                return [];
            });
    ...
    async webSearch(query: { [k: string]: any; }, opts?: ScrappingOptions) {
        ...
        const sideLoaded = await this.sideLoadWithAllocatedProxy(url, {
            ...opts,
            extraHeaders: {
                'User-Agent': ctx.ua || this.getGsaUserAgent(),
                ...opts?.extraHeaders,
            },
        ...
        try {
            const r = runGetWebSearchResultsScript2(createContext(jsdom));
            if (!Array.isArray(r)) {
...
export function runGetWebSearchResultsScript(ctx: object) {
    return script.runInContext(ctx);
}

export function getWebSearchResultsSync2() {
    const candidates = Array.from(document.querySelectorAll('a[data-ved]:not([class])'));

    return candidates.map((x, pos) => {

        const rawUrl = x.getAttribute('href');
        if (!rawUrl) {
            return undefined;
        }
        let url = rawUrl;
        if (rawUrl.startsWith('/url?q=')) {
            url = decodeURIComponent(rawUrl.substring(7).split('&sa=U')[0]);
        }
        const contentNodes = Array.from(x.parentElement?.parentElement?.querySelectorAll('div.ilUpNd.H66NU.aSRlid') || []);
        for (const n of contentNodes) {
            n.querySelectorAll('script').forEach(s => s.remove());
        }

        const date = contentNodes[0]?.querySelector('span.UK5aid.MDvRSc')?.textContent?.trim();
        const title = x.querySelector('div[style]')?.textContent;
        if (!title) {
            return undefined;
        }
        for (const node of contentNodes) {
            node.querySelectorAll('span').forEach((s) => s.remove());
        }
        const snippet = contentNodes[0]?.textContent || '';
...
const script2 = new Script(`(${getWebSearchResultsSync2.toString()})()`);
function runGetWebSearchResultsScript2(ctx: object) {
    return script2.runInContext(ctx);
}
